...
- [동시성 제어 방식]
  - 낙관적 락 : 낙관적 락은 충돌 빈도가 적고, 동시 요청 중 1건만 성공하는 케이스에 적합하다.(ex. 티켓 예약시스템)
  ...
  - 비관적 락 : 비관적 락은 충돌 빈도가 많고, 순차적으로 처리해야 하는 경우 적합(ex. 재고차감, 금융시스템)
  ...
  - 분산락 : Redis를 활용해서 분산락을 설정(Pub/Sub 방식)
    - 구현복잡도 : 
    - 성능
    - 효율성
