...
import com.tdd.concert.domain.seat_pessimistic.component.SeatReaderP;
...
@SpringBootTest
@Slf4j
public class ReservationSeatPessimisticLockTest {

  @Autowired
  private ReserveSeatPessimisticLock reserveSeatPessimisticLock;
  ...
  @Autowired
  private SeatJpaRepositoryP seatJpaRepositoryP;

  @Autowired
  private SeatReaderP seatReaderP;
  ...
  void setUp() {
    ...
    // 3. 좌석정보 생성
    seatP = seatJpaRepositoryP.save(
        SeatP.builder()
            .seatNo(1L)
            .price(1000)
            .seatStatusP(SeatStatusP.AVAILABLE)
  ...
  void case1() throws InterruptedException {
    ...
    for (int i = 0; i < threadCnt; i++) {
      ...
      CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
        try {
          reserveSeatPessimisticLock.reserve(reservationRequestList.get(finalI1));
    ...
    SeatP actualSeatP = seatJpaRepositoryP.findSeatPBySeatId(seatP.getSeatId());

    assertEquals(1, successCnt.intValue());
    assertEquals(threadCnt - successCnt.intValue(), failCnt.intValue());
    assertEquals(SeatStatusP.TEMPORARY_RESERVED, actualSeatP.getSeatStatusP());
  }
